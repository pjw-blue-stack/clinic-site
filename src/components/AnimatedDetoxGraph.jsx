...
export default function AnimatedDetoxGraph() {
  ...
  // 애니메이션이 스크롤의 80% 지점에서 완료되도록 매핑 (나머지 20%는 완성된 상태로 유지)
  const graphProgress = useTransform(scrollYProgress, [0, 0.85], [0, 1]);
  ...
  // Markers Opacity - adjusted for graphProgress mapping
  const startMarkerOpacity = useTransform(graphProgress, [0.15, 0.25, 1], [0, 1, 1]);
  const startMarkerY = useTransform(graphProgress, [0.15, 0.25], [20, 0]);
  
  const endMarkerOpacity = useTransform(graphProgress, [0.65, 0.75, 1], [0, 1, 1]);
  const endMarkerY = useTransform(graphProgress, [0.65, 0.75], [20, 0]);
  
  const finishMarkerOpacity = useTransform(graphProgress, [0.85, 0.95], [0, 1]);
  const finishMarkerY = useTransform(graphProgress, [0.85, 0.95], [20, 0]);

  return (
    <div ref={containerRef} style={{ height: '400vh', position: 'relative', width: '100%', maxWidth: '1000px', margin: '60px auto 0' }}>
      <div style={{ position: 'sticky', top: '10vh', height: '80vh', width: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <motion.div 
          style={{ 
            ...
            alignItems: 'center',
            opacity: useTransform(scrollYProgress, [0, 0.3, 1], [1, 0, 0])
          }}
        >
